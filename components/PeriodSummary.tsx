...
import { Team, Budget, ProductionEntry, MONTHS, YEARS, DAILY_WORKING_HOURS } from '../types';
...
const PeriodSummary: React.FC<PeriodSummaryProps> = ({ entries, budgets, teams }) => {
  ...
  const stats = useMemo(() => {
    ...
    // Dinamik Adam-Saat Hesabı
    const hours = periodBudgets.reduce((acc, curr) => {
      const days = curr.workingDays?.length || 22;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-lg overflow-hidden shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 divide-y md:divide-y-0 md:divide-x divide-slate-200">
          ...
          <SummaryBlock label="ADAM-SAAT" value={`${Math.round(stats.hours).toLocaleString()} SAAT`} icon={<Clock size={18} className="text-slate-500" />} />
        </div>
        ...
        <div className="p-8 border-t border-slate-100 bg-slate-50/50">
          ...
          <p className="text-xs text-slate-500 leading-relaxed font-medium">
            ...
            Adam-saat hesaplamaları, ekiplerin aylık takvimlerinde seçilen net çalışma günleri üzerinden <b>{DAILY_WORKING_HOURS}</b> saatlik mesai standardı baz alınarak hesaplanmıştır.
          </p>
        </div>
      </div>
    </div>
